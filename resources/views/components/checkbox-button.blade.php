@props(['id' => null, 'name' => null, 'checked' => false, 'required' => false, 'value' => $value, "xmodel" => null, "readonly" => false])

<input type="checkbox" name="{{$name}}" id="{{ $id }}" value="{{ $value }}" class="hidden peer" @checked($checked) @required($required) @if($xmodel)x-model="{{$xmodel}}"@endif @readonly($readonly)>
